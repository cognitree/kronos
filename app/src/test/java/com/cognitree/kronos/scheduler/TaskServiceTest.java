...
public class TaskServiceTest extends ApplicationTest {
    ...
    public void testGetTasksByNamespace() throws Exception {
        ...
        final List<Task> cognitreeTasks = taskService.get(workflowTriggerOne.getNamespace());
        Assert.assertEquals(3, cognitreeTasks.size());
        final List<Task> kronosTasks = taskService.get(workflowTriggerTwo.getNamespace());
        Assert.assertEquals(3, kronosTasks.size());
    }
